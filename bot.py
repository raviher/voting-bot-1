...
from handlers.channel_check import check_joined_callback, is_channel_member
    ...
    async def channel_check(update: Update, context) -> None:
        ...
        for channel in channels:
            try:
                ...
                # A failed lookup is not proof of membership. Block access
                # until the bot can verify the required channel.
                missing.append(channel)
        ...
        if update.message:
            ...
            try:
                ...
                # Always show a fresh prompt if this Telegram message cannot
                # be edited, so users can see and tap the channel join button.
